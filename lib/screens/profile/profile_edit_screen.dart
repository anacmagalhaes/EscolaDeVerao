...
class _ProfileEditScreenState extends State<ProfileEditScreen> {
  ...
  void _validateLinks() {
    setState(() {
      linkedinError = linkedinEditController.text.isNotEmpty &&
              !_isValidUrl(linkedinEditController.text)
          ? "Link do LinkedIn inválido"
          : null;
      lattesError = latesEditController.text.isNotEmpty &&
              !_isValidUrl(latesEditController.text)
          ? "Link do Lattes inválido"
          : null;
    });
  }
  ...
  Future<void> _updateProfile() async {
    _validateLinks();
    if (linkedinError != null || lattesError != null) {
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        setState(() {
          linkedinError = result['errors']?['linkedin']?.first;
          lattesError = result['errors']?['lattes']?.first;
        });
      }
    } catch (e, stackTrace) {
    ...
    return PopScope(
        ...
        onPopInvoked: (didPop) {
          ...
          if (Navigator.canPop(context)) {
            Navigator.pop(
                context); // Volta normalmente se houver telas na pilha
          } else {
              ...
              child: Scaffold(
                appBar: PreferredSize(
                  ...
                  child: Container(
                    ...
                    child: Padding(
                      padding:
                          EdgeInsets.only(left: 10.h, right: 10.h, top: 35.h),
                      child: Row(
                        children: [
                          GestureDetector(
                            onTap: () {
                              if (widget.origin == 'settings') {
                                ...
                                Navigator.pop(
                                    context); // Fallback para outras telas
                              }
                            },
                            child: SizedBox(
                              width: 44.h,
                              height: 44.h,
                              child: Image.asset(
                                  'assets/icons/angle-left-white.png'),
                            ),
                          ),
                        ],
                      ),
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            children: [
                              SizedBox(height: 30.h), // Space for profile image
                              Fonts(
